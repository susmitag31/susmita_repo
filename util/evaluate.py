...
#out_file=open("output_questions.txt","w")
   ...
   # print('Evaluation start!')
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(loader):
            ...
            c=1 
            if c==1:# batch_idx == 0 and (not test):
                # if batch_idx > 260:
                ...
                #    out_file.write(f'[{i}]')
                 ...
                 #   out_file.write(f"Source Sequence: {' '.join(source_WORD[i])}")        
                    if config.task == 'QG':
                        print(f"Answer: {' '.join(answer_WORD[i])}")
                  #      out_file.write(f"Answer: {' '.join(answer_WORD[i])}")
                    if config.use_focus:
                        print(f"Oracle Focus: {' '.join(focus_WORD[i])}")
                   #     out_file.write(f"Oracle Focus: {' '.join(focus_WORD[i])}")
                    if config.task == 'QG':
                    ...
                    if config.n_mixture > 1:
                        for n in range(config.n_mixture):
                            if config.use_focus:
                                print(f'(focus {n})')
                     #           out_file.write(f"(focus {n})")

      #                          print(
       #                             f"Focus Prob: {' '.join([f'({w}/{p:.2f})' for (w, p) in zip(s, f_p[n])])}")
                                print( f"Generated Focus: {' '.join([w for w, m in zip(s, g_m[n]) if m == 1])}")
                      #          out_file.write( f"Generated Focus: {' '.join([w for w, m in zip(s, g_m[n]) if m == 1])}")
                            if config.task == 'QG':
                                print(f"Generated Question: {' '.join(hypotheses[n][B * batch_idx + i])}\n")
                       #         out_file.write(f"Generated Question: {' '.join(hypotheses[n][B * batch_idx + i])}\n")
                            elif config.task == 'SM':
                                print(
...
if __name__ == '__main__':
 ...
 #   print(config)
    ...
    from build_utils1 import get_loader, build_model, get_ckpt_name
